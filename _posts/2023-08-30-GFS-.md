...
我不会一字一句地翻译这篇论文，我打算将一些关键点提炼出来以供学习。  
...
数据变化可能是写入或追加记录。写操作导致在应用程序指定的文件偏移量处写入数据。记录追加会导致数据(“记录”)至少自动追加一次，即使在存在并发突变的情况下也是如此，但是以GFS选择的偏移量进行追加。(相反，“常规”追加只是在客户端认为是文件当前结束的偏移量处进行写操作。)  
